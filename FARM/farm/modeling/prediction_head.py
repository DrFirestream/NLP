...
from farm.modeling.heinsen_routing import Routing
...
class CapsuleTextClassificationHead(PredictionHead):
    def __init__(
        self, n_classes,
        d_depth = 14, d_emb = 768, d_inp=64, d_cap=2, n_parts=64, n = None,
        class_weights=None,
        loss_ignore_index=-100,
        loss_reduction="none",
        task_name="text_classification",
        **kwargs,
        ...
        :param kwargs:
        """
        super(CapsuleTextClassificationHead, self).__init__()
        # num_labels could in most cases also be automatically retrieved from the data processor
        self.routing = not n
        self.n = n
        self.n_classes = n_classes
        depth = 1#d_depth
        self.detect_parts = nn.Sequential(nn.Linear(d_emb, d_inp), Swish(), nn.LayerNorm(d_inp)) if self.routing else nn.Sequential(nn.Linear(n*depth * d_emb,  n_classes)) #nn.LayerNorm(n*d_emb * depth)
        ...
        nn.init.kaiming_normal_(self.detect_parts[0].weight if self.routing else self.detect_parts[0].weight)
        nn.init.zeros_(self.detect_parts[0].bias if self.routing else self.detect_parts[0].bias)
        self.ph_output_type = "per_sequence"
        self.model_type = "capsule_text_classification"
        self.task_name = task_name #used for connecting with the right output of the processor
        self.class_weights = class_weights
    ...
    def forward(self, embs, mask):
        if self.routing:
            if mask is None:
                a = torch.log(1.0/(embs[:,:,:,0] * 0)).view(embs.shape[0], -1)
            else:
            ...
            mu = self.detect_parts(self.dropout(embs) + self.depth_emb)          # [bs, n, d_depth, d_inp]
            mu = mu.view(mu.shape[0], -1, 1, mu.shape[-1])         # [bs, (n * d_depth), 1, d_inp]

            for routing in self.routings:
                a, mu, sig2 = routing(a, mu)
        else:
            #a = self.dropout(embs) + self.depth_emb
            #a = a.view(a.shape[0], -1)
            a = torch.flatten(self.dropout(embs[:,0:self.n,-1,:]), 1)
            a, mu, sig2 = self.detect_parts(a), None, None
        return a

    def logits_to_loss(self, logits, **kwargs):
        #onehot = torch.eye(self.n_classes, device=logits.device)
        label_ids = kwargs.get(self.label_tensor_name)
        #losses = -onehot[label_ids] * nn.functional.log_softmax(logits, dim=-1)  # CE
