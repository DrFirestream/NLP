...
class AdaptiveModel(nn.Module, BaseAdaptiveModel):
    ...
    def __init__(
        ...
        for head in self.prediction_heads:
            ...
            elif head.model_type.startswith("capsule"):
                self.has_capsules = True
    ...
    def forward(self, **kwargs):
        ...
        if len(self.prediction_heads) > 0:
            for head, lm_out in zip(self.prediction_heads, self.lm_output_types):
                if head.model_type.startswith("capsule"):
                    output = torch.stack(hidden_states[::-1]+(sequence_output,), dim=-2)
                    all_logits.append(head(output, kwargs.get('padding_mask', None)))
                    continue
    ...
    def forward_lm(self, **kwargs):
        ...
        # Run forward pass of language model
        if extraction_layer == -1 and not self.has_capsules:
