...
class Model(nn.Module):
    ...
    def forward(self, x, past=None, mask = None):
        ...
        presents = []
        hidden = [h]
        #return {'hidden': torch.stack(tuple(hidden), dim=-2)}
        for i, block in enumerate(self.blocks):
            if self.hparams.gradient_checkpointing:
                h, present = torch.utils.checkpoint.checkpoint(block, h, past[:, i] if past is not None else None, mask)
            else:
                h, present = block(h, past=past[:, i] if past is not None else None, mask=mask)
        ...
        return {
            ...
            'logits': logits,
            'hidden': torch.stack(tuple(hidden), dim=-2)
        }
...
class Block(nn.Module):
    ...
    def forward(self, x, past, mask = None):
        a, present = self.attn(self.ln_1(x), past=past, mask=mask)
        x = x + a
...
class Attention(nn.Module):
    ...
    def mask_attn_weights(self, w, mask = None):
        ...
        if not mask is None:
           mask = mask.unsqueeze(1).unsqueeze(1).repeat(1,1,nd,1)
           b = mask * b
        w = w * b - 1e10 * (1 - b)
        return w
    ...
    def multihead_attn(self, q, k, v, mask = None):
        # q, k, v have shape [batch, heads, sequence, features]
        w = torch.matmul(q, k.permute(0, 1, 3, 2))
        w = w / math.sqrt(v.shape[-1])
        w = self.mask_attn_weights(w, mask)
        w = F.softmax(w, dim=-1)
        a = torch.matmul(w, v)
        return a
