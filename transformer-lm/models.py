...
class SSTClassifier(nn.Module):
    ...
    """
    def __init__(self, d_depth, d_emb, d_inp, d_cap, n_parts, n_classes, n = None):
        super().__init__()
        self.routing = not n
        self.n = n
        depth = 1#d_depth
        self.detect_parts = nn.Sequential(nn.Linear(d_emb, d_inp), Swish(), nn.LayerNorm(d_inp)) if self.routing else nn.Sequential(nn.LayerNorm(n*d_emb * depth), nn.Linear(n*depth * d_emb,  n_classes))
        ...
        nn.init.kaiming_normal_(self.detect_parts[0].weight if self.routing else self.detect_parts[1].weight)
        nn.init.zeros_(self.detect_parts[0].bias if self.routing else self.detect_parts[1].bias)

    def forward(self, mask, embs):
        #embs = self.dropout(embs[:,:,0:1,:])
        if self.routing:
